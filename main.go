...
func main() {
	...
	flagset.BoolVar(&disableRulesFilter, "disable-rules-filter", false, "When specified the proxy will no filter prometheus rule based on labels")
	...
	if disableRulesFilter {
		opts = append(opts, injectproxy.WithDisableRulesFilter(true))
	}
